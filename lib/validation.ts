...
// Constants for validation - can be overridden by environment variables
export const VALIDATION_LIMITS = {
  MAX_FILE_SIZE: (parseInt(process.env.NEXT_PUBLIC_MAX_FILE_SIZE_MB || '50') * 1024 * 1024), // Default 50MB per file
  MAX_TOTAL_SIZE: (parseInt(process.env.NEXT_PUBLIC_MAX_TOTAL_SIZE_MB || '500') * 1024 * 1024), // Default 500MB total
  MAX_FILES: parseInt(process.env.NEXT_PUBLIC_MAX_FILES || '50'), // Default 50 files
  MAX_PAGES_PER_FILE: parseInt(process.env.NEXT_PUBLIC_MAX_PAGES_PER_FILE || '500'), // Default 500 pages per file
  MAX_TOTAL_PAGES: parseInt(process.env.NEXT_PUBLIC_MAX_TOTAL_PAGES || '2000'), // Default 2000 total pages
  MIN_DPI: 72,
  MAX_DPI: 600,
...
export function validateFilename(filename: string): ValidationError | null {
  ...
  if (invalidChars.test(filename)) {
    return {
      type: 'filename',
      message: 'Filename contains invalid characters. Please use only letters, numbers, spaces, hyphens, and underscores.'
    }
  }
...
export function validateSettings(settings: {
  ...
  if (!Number.isInteger(settings.pagesPerSheet) || settings.pagesPerSheet < 1 || settings.pagesPerSheet > 9) {
    errors.push({
      type: 'settings',
  ...
  // Validate DPI
  if (!Number.isInteger(settings.dpi) || settings.dpi < VALIDATION_LIMITS.MIN_DPI || settings.dpi > VALIDATION_LIMITS.MAX_DPI) {
    errors.push({
      type: 'settings',
...
/**
 * Validate page count for processing
 */
export function validatePageCount(totalPages: number, pagesPerSheet: number): ValidationError | null {
