...
export function SortableFileItem({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className={`group flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-3 sm:py-3 hover:bg-muted/50 transition-colors border-b border-border last:border-b-0 ${isDragging ? 'opacity-50' : ''
    ...
    >
      ...
      <div
        {...(canDrag ? attributes : {})}
        {...(canDrag ? listeners : {})}
        className={`hidden sm:flex shrink-0 text-muted-foreground/50 group-hover:text-muted-foreground transition-colors ${canDrag ? 'cursor-grab active:cursor-grabbing' : 'cursor-default'
      ...
      {/* Mobile: Whole element draggable */}
      <div
        {...(canDrag ? attributes : {})}
        {...(canDrag ? listeners : {})}
        className={`sm:hidden flex-1 min-w-0 flex items-center gap-2 ${canDrag ? 'cursor-grab active:cursor-grabbing' : 'cursor-default'
          }`}
        style={{ touchAction: canDrag ? 'none' : 'auto' }}
      >
        <FileIcon size="sm" />
      ...
      {/* Desktop: File Icon & Info (not draggable) */}
      <div className="hidden sm:flex items-center gap-3 flex-1 min-w-0">
        <FileIcon size="md" />
        <FileInfo file={file} size="md" />
      </div>

      {/* Remove Button - Always visible on touch, hover on mouse */}
      <button
...
function FileIcon({ size }: { size: 'sm' | 'md' }) {
  ...
  return (
    <div className={`shrink-0 ${sizeClasses} bg-primary/10 flex items-center justify-center text-primary`}>
