    ...
    >
      <div className="space-y-3 sm:space-y-4">
        ...
        <div className="text-sm text-muted-foreground tabular-nums">
          <Balancer>{state.message}</Balancer>
        </div>
      </div>
        ...
        <div className="w-full bg-muted rounded-full h-2 sm:h-3 overflow-hidden">
          ...
          >
            {state.progress > 0 && state.progress < 100 && (
              <div className="absolute top-0 right-0 w-4 h-full bg-linear-to-r from-transparent to-white/40 rounded-r-full animate-pulse" />
            )}
          </div>
        </div>
      </div>
      <div className="bg-muted/50 rounded-xl p-3 sm:p-4 text-left">
        <div className="flex items-center gap-2 mb-2">
          <HugeiconsIcon icon={Lightbulb} className="w-3 h-3 sm:w-4 sm:h-4 text-primary shrink-0" />
